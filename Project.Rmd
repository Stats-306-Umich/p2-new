...
```{r parallel-genre-counts, message=FALSE, warning=FALSE}
library(data.table)
library(data.table)
library(parallel)

#–– Prepare the long‐form genre table (if you haven’t already)
dt <- as.data.table(title_basics)
titles_genre_dt <- dt[
  !is.na(startYear) & genres != "",
  .(
...
years <- unique(titles_genre_dt$startYear)

#–– Create a fork cluster (inherits loaded packages & objects!)
ncores <- max(detectCores() - 1, 1)
cl     <- makeForkCluster(ncores)

#–– Serial timing
t_serial <- system.time({
  genre_counts_serial <- titles_genre_dt[, .(count = .N), by = .(startYear, genres)]
})

#–– Parallel timing
t_parallel <- system.time({
  year_lists <- parLapply(cl, years, function(yr) {
    sub <- titles_genre_dt[startYear == yr]
...
```
The regex_count version runs roughly 3× faster (median ≈1.6 ms vs. ≈5.0 ms) and is more concise. Recommendation: use str_count(x, ",") + 1 for counting comma-separated items.
