...
eda_summary <- function(df, df_name) {
  ...
  if (any(char_cols)) {
    ...
    for (colname in names(df)[char_cols]) {
      ...
      if (n_unique <= 30) {  # You can adjust threshold if needed
...
**The most common pair is genreNA and genresFilm-Noir**
```{r}
genre_matrix <- model.matrix(~ genres - 1, data = titles_genre_expanded)

# 2. Calculate the correlation matrix
genre_cor_matrix <- cor(genre_matrix)

# 3. Find the most common pair of genres
most_common_pair <- which(genre_cor_matrix == max(genre_cor_matrix[lower.tri(genre_cor_matrix)]), arr.ind = TRUE)
